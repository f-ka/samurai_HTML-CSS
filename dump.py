...
def calc_features(hist):
    ...
    hist["SMA5"] = talib.SMA(close, timeperiod=5)
    hist["SMA25"] = talib.SMA(close, timeperiod=25)
...
for file in files:
    ...
    diff = hist[code]["SMA5"] - hist[code]["SMA25"]
...
hist["pred_Return"] = model.predict(hist[features])
